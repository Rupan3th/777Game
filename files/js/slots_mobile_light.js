...
wLaoHuJi.prototype.result = function () {
    ...
    if (dice_point > 10) {
        taisai = 1;
        this._money += parseInt((document.getElementById("dice_bet_txt_big").value) * bet_rate);
    }
    else {
        taisai = 0;
        this._money += parseInt((document.getElementById("dice_bet_txt_small").value) * bet_rate);
    }
    if (dice_point % 2 == 0) {
        evodd = 0;
        this._money += parseInt((document.getElementById("dice_bet_txt_even").value) * bet_rate);
    }
    else {
        evodd = 1;
        this._money += parseInt((document.getElementById("dice_bet_txt_odd").value) * bet_rate);
    }
    bet_state = 0;
    this.$('lhj_ben_txt_money').innerHTML = this._money;
    display_segment("txt_money_7dgt", this._money, 12, 15, 36, 4);
...
wLaoHuJi.prototype.init = function () {
    ...
    mainhtml = "<div style = 'position: relative;top:0px;left:0px;width:" + piecewidth + "px;height:" + pieceheight + "px;'><div id='lhj_piece_bg' style='position:absolute;top:0,left:0;'></div><div id='lhj_piece_run' style='position:absolute;top:0,left:0;'></div></div><div id='lhj_bet'> </div>";
    ...
    bethtml.push("<div id='bet_area' style='position:absolute;width:720px;height:650px;z-index:102;left:0px;top:70px;  background:url(/files/image/images_m/bet_area_cl.png); opacity:1;  display: none '>");
    ...
    bethtml.push("<image id='red_lamp' width='25' height='25' src ='/files/image/images/led-green.png' style='left:300px; top:250px; position: absolute '/>");
    ...
    setInterval(function () {
        ...
        if (sectime == 110) {
            ...
            display_segment("txt_money_7dgt", self._money, 12, 15, 36, 4);      
            display_segment("txt_total_7dgt", self._total, 12, 15, 36, 4);
