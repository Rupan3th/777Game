...
public partial class media_movie : System.Web.UI.Page
{
    ...
    static string sqlForPage = "SELECT id, mvname, pathname, admin_id, i_url, v_url, coin, clss_id, hao, bad, rcnt, regdate, isdel FROM movie WHERE (isdel = 0) ORDER BY regdate DESC";
    //}}This Code For paging process about datalist control

    protected void Page_Load(object sender, EventArgs e)
    {
        if(Request["preview"] != null) pnlSearch.Visible = false;
        ...
        if (!Page.IsPostBack)
        {
            ...
            {
                sqlForPage = "SELECT id, mvname, pathname, admin_id, i_url, v_url, coin, clss_id, hao, bad, rcnt, regdate, isdel FROM movie WHERE (isdel = 0) and clss_id=" + Page.Request["clss"].ToString() + " ORDER BY regdate DESC";
            }
            BindData();
        }
    }
    ...
    {
        sqlForPage = "SELECT id, mvname, pathname, admin_id, i_url, v_url, coin, clss_id, hao, bad, rcnt, regdate, isdel FROM movie WHERE (isdel = 0)";
        if (ddlKInd.Text.Length > 0) sqlForPage += " and clss_id = " + ddlKInd.SelectedValue;
        if (tbSearch.Text.Length > 0) sqlForPage += " and mvname like '%"+tbSearch.Text+"%'";
